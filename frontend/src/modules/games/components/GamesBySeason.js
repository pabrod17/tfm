...
const GameCardUser = ({ dispatch, history, seasonId, item, handleOpenDescriptionModal }) => {
  return (
    <div key={item.id}>
      <div>
  ...
  function GamesListUser({seasonId, items, exercisesList, stretchingsList, fallback, dispatch, history, handleOpenDescription }) {
    if (!items || items.length === 0) {
      // dispatch(actions.findGamesBySeasonId(seasonId, () => history(`/seasons/update/${seasonId}/game/${3}`)));
      // return fallback;
    } else {
      return items.map(item => (
        <GameCardUser dispatch={dispatch} history={history} seasonId={seasonId} item={item} handleOpenDescriptionModal={handleOpenDescription} />
      ));
    }
  }


const GamesBySeason = ({games, seasonId}) => {
    ...
    const handleClose = () => {
      ...
      setOpenDescription(false);
        ...
        return(
          ...
          <GamesListUser seasonId={seasonId} items={games} fallback={"Loading..."} dispatch = {dispatch} history={history} handleOpenDescription={handleOpenDescription}/>
          {(openDescription) && (
