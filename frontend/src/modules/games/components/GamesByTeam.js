...
const GameCardUser = ({ dispatch, history, teamId, item, handleOpenDescriptionModal }) => {
  return (
    <div key={item.id}>
      <div>
  ...
  function GamesListUser({teamId, items, exercisesList, stretchingsList, fallback, dispatch, history, handleOpenDescription }) {
    if (!items || items.length === 0) {
      // dispatch(actions.findGamesBySeasonId(seasonId, () => history(`/seasons/update/${seasonId}/game/${3}`)));
      // return fallback;
    } else {
      return items.map(item => (
        <GameCardUser dispatch={dispatch} history={history} teamId={teamId} item={item} handleOpenDescriptionModal={handleOpenDescription} />
      ));
    }
  }


const GamesByTeam = ({games, teamId}) => {
    ...
    const handleClose = () => {
      ...
      setOpenDescription(false);
        ...
        return(
          ...
          <GamesListUser teamId={teamId} items={games} fallback={"Loading..."} dispatch = {dispatch} history={history} handleOpenDescription={handleOpenDescription}/>
          {(openDescription) && (
