...
const handleRemoveLesionToPlayer = (id, playerId, dispatch, history) => {
    dispatch(actions.removeLesionToPlayer(playerId, id, () => reloadWindow(id, playerId, history)));
}
const reloadWindow = (id, playerId, history) =>{
  history(`/players/update/${id}/lesion/${playerId}`);
...
function LesionsList({ items, playerId, fallback, dispatch, history, openDescription, openMedication, handleOpenDescription, handleOpenMedication, handleClose }) {
  if (!items || items.length === 0) {
    // dispatch(actions.findAllLesion(() => history('/lesion/home')));
    // return fallback;
  } else {
    return items.map(item => (
      <LesionCard dispatch={dispatch} playerId={playerId} history={history} key={item.id} item={item} handleOpenDescriptionModal={handleOpenDescription} handleOpenMedicationModal={handleOpenMedication} />
    ));
  }
}
...
LesionsByPlayer.propTypes = {
  lesionsByPlayer: PropTypes.array
};
