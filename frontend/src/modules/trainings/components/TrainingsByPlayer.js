...
const TrainingCardUser = ({ dispatch, history, playerId, item, handleOpenDescriptionModal }) => {
  return (
    <div key={item.id}>
      <div>
...
function TrainingsListUser({playerId, items, exercisesList, stretchingsList, fallback, dispatch, history, handleOpenDescription }) {
  if (!items || items.length === 0) {
    // dispatch(actions.findTrainingsBySeasonId(seasonId, () => history(`/seasons/update/${seasonId}/training/${1}`)));
    // return fallback;
  } else {
    return items.map(item => (
      <TrainingCardUser dispatch={dispatch} history={history} playerId={playerId} item={item} handleOpenDescriptionModal={handleOpenDescription} />
    ));
  }
}

const TrainingsByPlayer = ({trainings, playerId}) => {
    ...
    const handleClose = () => {
      ...
      return(
        ...
        <TrainingsListUser playerId={playerId} items={trainings} fallback={"Loading..."} dispatch = {dispatch} history={history} handleOpenDescription={handleOpenDescription}/>
        {(openDescription) && (
...
export default TrainingsByPlayer;