...
const TrainingCardUser = ({ dispatch, history, teamId, item, handleOpenDescriptionModal }) => {
  return (
    <div key={item.id}>
      <div>
...
function TrainingsListUser({teamId, items, exercisesList, stretchingsList, fallback, dispatch, history, handleOpenDescription }) {
  if (!items || items.length === 0) {
    // dispatch(actions.findTrainingsBySeasonId(seasonId, () => history(`/seasons/update/${seasonId}/training/${1}`)));
    // return fallback;
  } else {
    return items.map(item => (
      <TrainingCardUser dispatch={dispatch} history={history} teamId={teamId} item={item} handleOpenDescriptionModal={handleOpenDescription} />
    ));
  }
}

const TrainingsByTeam = ({trainings, teamId}) => {
    ...
    const handleClose = () => {
      ...
      return(
        ...
        <TrainingsListUser teamId={teamId} items={trainings} fallback={"Loading..."} dispatch = {dispatch} history={history} handleOpenDescription={handleOpenDescription}/>
        {(openDescription) && (
...
export default TrainingsByTeam;