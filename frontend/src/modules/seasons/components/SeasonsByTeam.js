...
const CardSeason = ({ dispatch, teamId, history, item, handleOpenDescriptionModal }) => {
...
function SeasonsList({ items, teamId, fallback, dispatch, history, handleOpenDescription }) {
  if (!items || items.length === 0) {
    dispatch(actions.findSeasonsToTeam(teamId, () => history(`/teams/update/${teamId}/season/${3}`)));
    return fallback;
  } else {
    return items.map(item => (
      <CardSeason dispatch={dispatch} teamId={teamId} history={history} key={item.id} item={item} handleOpenDescriptionModal={handleOpenDescription} />
    ));
  }
}

const SeasonsByTeam = ({seasons, teamId}) => {
    ...
    return(
        <div class="card-group lesions_contaner">
            <SeasonsList items={seasons} teamId={teamId} fallback={"Loading..."} dispatch = {dispatch} history={history} handleOpenDescription={handleOpenDescription} />
            {(openDescription) && (
...
SeasonsByTeam.propTypes = {
    seasons: PropTypes.array
};

export default SeasonsByTeam;