...
const UpdatePlayerStretching = () => {
    ...
    const stretchingList = useSelector(selectorsStretchings.getStretchingsByPlayerId);
    const stretchingListAll = useSelector(selectorsStretchings.getAllStretchings);


      useEffect(() => {
        if (!stretchingList) {
            dispatch(actionsStretching.findStretchingsByPlayerId(id, () => history(`/players/update/${id}/stretching/${5}`)));
            dispatch(actions.findPlayerById(id, () => history(`/players/update/${id}/stretching/${5}`)));
        }
    }, [dispatch, stretchingList, history, id]);
    useEffect(() => {
            if (!stretchingListAll.stretchings) {
                dispatch(actionsStretching.findAllStretchings(() => history(`/players/update/${id}/stretching/${5}`)));
            } else {
                ...
                if (filteredStretchings) {
                    const newRowsStretching = filteredStretchings.map(stretching => ({
                        id: stretching.id,
                        name: stretching.stretchingName,
                        type: stretching.stretchingType,
                        description: stretching.description,
                    }));
                    setRowsStretchings(newRowsStretching);
                }
        ...
        const handleSubmit = event => {
            ...
            dispatch(actionsStretching.addStretchingToPlayer(id, stretchingIds,
...
<Box sx={{boxShadow:"0 10px 50px rgb(0, 0, 0)" }}>
        ...
        >
          ...
          <Tab value={3} sx={{ color: '#d17403', fontSize: "30px", padding:"20px" }} onClick={() => handleUpdatePlayerTrainings(3, dispatch)} label="Trainings"/>
    ...
    {showTable && (
        ...
        >
            ...
            <button className="post_player" type='submit' onClick={(e) => handleSubmit(e)}><FormattedMessage id="project.global.buttons.save" /></button>
                ...
                <StretchingsByPlayer stretchings={stretchingList} playerId={id} />
...
export default UpdatePlayerStretching;