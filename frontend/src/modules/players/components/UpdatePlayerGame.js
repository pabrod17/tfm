...
const UpdatePlayerGame = () => {
    ...
    const gamesList = useSelector(selectorsGames.getGamesByPlayerId);


      useEffect(() => {
        if (!gamesList) {
            dispatch(actionsGames.findGamesByPlayerId(id, () => history(`/players/update/${id}/game/${2}`)));
            dispatch(actions.findPlayerById(id, () => history(`/players/update/${id}/game/${2}`)));
        }
...
<Box
            ...
            >
                ...
                <GamesByPlayer games={gamesList} playerId={id} />
...
export default UpdatePlayerGame;