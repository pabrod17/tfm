...
// }
    ...
    function PlayersList({ items, gameId, fallback, dispatch, history, openDescription, handleOpenDescription}) {
      if (!items || items.length === 0) {
        // dispatch(actions.findStretchingsByTrainingId(trainingId, () => history(`/trainings/update/${trainingId}/stretching`)));
        // return fallback;
      } else {
        return items.map(item => (
          <PlayerCard dispatch={dispatch} gameId={gameId} history={history} key={item.id} item={item} handleOpenDescriptionModal={handleOpenDescription} />
        ));
      }
    }


const PlayersByGameStatistics = ({players, gameId}) => {
    ...
    return(
        <div className="card-group lesions_contaner">
          <PlayersList items={players} gameId={gameId} fallback={"Loading..."} dispatch = {dispatch} history={history} openDescription={openDescription} handleOpenDescription={handleOpenDescription} />
          {(openDescription) && (
...
PlayersByGameStatistics.propTypes = {
    playersByGame: PropTypes.array
};

export default PlayersByGameStatistics;