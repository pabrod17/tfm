...
const handleFindTrainingsToPlayer = (playerId, dispatch, history) => {
  ...
  const handleRemovePlayer = (playerId, teamId, dispatch, history) => {
    dispatch(actions.removePlayer(playerId, teamId, () => window.location.reload()));
  }
...
// }
    ...
    const PlayerCard = ({ dispatch, teamId, history, item, handleOpenDescriptionModal, handleOpenMedicationModal }) => {
      return (
        <div key={item.id}>
          <div>
              ...
              <div className="flip-card-inner">
                ...
                <div className="flip-card-back">
                  ...
                  <ul class="social-icons trashgrande trash_position">
                    <li><a type="button" onClick={() => handleRemovePlayer(item.id, teamId, dispatch, history)}>
                      <i class="fa fa-trash"></i></a></li>
                  </ul>
                </div>
              </div>
    ...
    function PlayersList({ items, teamId, fallback, dispatch, history, openDescription, handleOpenDescription}) {
      if (!items || items.length === 0) {
        // dispatch(actions.findStretchingsByTrainingId(trainingId, () => history(`/trainings/update/${trainingId}/stretching`)));
        // return fallback;
      } else {
        return items.map(item => (
          <PlayerCard dispatch={dispatch} teamId={teamId} history={history} key={item.id} item={item} handleOpenDescriptionModal={handleOpenDescription} />
        ));
      }
    }


const PlayersByTeam = ({players, teamId}) => {
    ...
    return(
        <div className="card-group lesions_contaner">
          <PlayersList items={players} teamId={teamId} fallback={"Loading..."} dispatch = {dispatch} history={history} openDescription={openDescription} handleOpenDescription={handleOpenDescription} />
          {(openDescription) && (
