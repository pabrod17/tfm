...
import { SeasonsByTeam } from '../../seasons';

const UpdateTeamSeason = () => {
    ...
    const seasonsList = useSelector(selectorsSeasons.getAllSeasons);

    useEffect(() => {
        if (!seasonsList) {
            dispatch(actionsSeasons.findSeasonsToTeam(id, () => history(`/teams/update/${id}/season/${3}`)));
            dispatch(actions.findTeamById(id, () => history(`/teams/update/${id}/season/${3}`)));
        }
    }, [dispatch, seasonsList, history, id]);
    ...
    return (
        ...
        >
            ...
            >
                <SeasonsByTeam seasons={seasonsList.seasons} teamId={id} />
...
export default UpdateTeamSeason;